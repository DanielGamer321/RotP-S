package com.danielgamer321.rotp_sp.network.packets.fromserver;
...
public class CommonConfigPacket {
    private final RotpSpConfig.Common.SyncedValues values;
    
    public CommonConfigPacket(RotpSpConfig.Common.SyncedValues values) {
        this.values = values;
    }
    ...
    public static class Handler implements IModPacketHandler<CommonConfigPacket> {
        ...
        public CommonConfigPacket decode(PacketBuffer buf) {
            return new CommonConfigPacket(new RotpSpConfig.Common.SyncedValues(buf));
        }
